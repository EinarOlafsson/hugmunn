"""Free what localagent owns, and nothing else.
...
``empty_cache()``. localagent holds no VRAM at all — llama-server does, in a
...
def clear_ram() -> Reclaim:
    """Drop localagent's own caches and collect. RSS measured either side."""
    before = process_rss()
    details: list[str] = []
    ...
    for name, module in list(sys.modules.items()):
        if not name.startswith("localagent."):
            continue
...
def clear_vram(server=None) -> Reclaim:
    """Stop the model server, which is the only VRAM localagent holds.

    ``server`` is a :class:`~localagent.core.server.ServerManager`. A server
...
def disk_report(paths=None) -> list[DiskEntry]:
    """Free space on every drive localagent touches. Reads; frees nothing.
...
CONFIRMATIONS: dict[str, tuple[str, str]] = {
    ...
    "vram": ("Release VRAM", (
        "localagent will stop the running model server, which unloads the "
        ...
        "server that was already running before localagent started is left "
        "alone, because stopping it is not this button's decision."
    )),
    "cpu": ("Release CPU", (
        "localagent will let Qt retire the worker threads it has finished with.\n\n"
        "No process is killed and no request is cancelled — not localagent's, "
        ...
        "capacity localagent is holding and not using."
    )),
    "disk": ("Check disk space", (
        "localagent will read the free space on every drive it touches — where "
