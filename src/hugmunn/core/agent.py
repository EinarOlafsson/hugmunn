...
from . import results as resultkit
...
class Agent:
    def __init__(
        ...
        results: resultkit.ResultStore | None = None,
        ...
        # Large tool output is set aside here and replaced in the conversation
        # by a digest plus a handle. One read_file on a real source file is
        # several thousand tokens, and the model usually wanted one function.
        self.results = results if results is not None else resultkit.ResultStore()
        self.extra_tools.append(resultkit.make_recall_tool(self.results))
        self.client = client
    ...
    def run(
        ...
        for iteration in range(self.max_iterations):
            ...
            for call in calls:
                ...
                # recall's own output is already bounded, and digesting it
                # would produce a handle to a handle.
                if call.name != "recall":
                    output = self.results.digest(call.name, summary, output)
